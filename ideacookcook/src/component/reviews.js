...
class Reviews extends Component {
  ...
  goDel = (index) => {
    this.setState({
      L: this.state.L.filter((x, i) => i != index),
      run: false,
      more: false,
    });
  ...
  edit = () => {
    return (
      ...
      >
        ...
        <textarea className="myComment" onChange={this.change}>
          {this.state.comment}
        </textarea>
      </div>
    );
  ...
  RVs = (L) => {
          ...
          {!(this.state.editing && this.state.editIndex == index) ? (
            <div className="comment">
              ...
              <div
                className="goDel"
                style={
                  review.MemID == this.props.myID && this.state.more
                    ? { visibility: "visible" }
                    : { visibility: "hidden" }
                }
                onClick={() => this.goDel(index)}
              >
                delete
              </div>
              <div
                ...
                style={
                  review.MemID == this.props.myID && this.state.more
                    ? { visibility: "visible", transform: "translateY(25px)" }
                    : { visibility: "hidden" }
                }
                onClick={() =>
                  this.setState({
    ...
    return <div className="reviews">{listRVs}</div>;
  };
  ...
  render() {
    ...
    return <div>{this.RVs(this.state.L)}</div>;
  }
}
